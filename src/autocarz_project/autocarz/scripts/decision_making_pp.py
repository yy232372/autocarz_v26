...
class PathPlanner:
    ...
    def find_nearest_waypoint_stanley(self, path, curX, curY):
        ...
        for i, pose in enumerate(path.poses):
            ...
            if dist < minDist:
                mdx = dx
                mdy = dy
                yaw_x = path.poses[(i + 1) % path_len].pose.position.x - path.poses[i].pose.position.x
                yaw_y = path.poses[(i + 1) % path_len].pose.position.y - path.poses[i].pose.position.y
                nearest_idx = i
                minDist = dist
    ...
    def localization(self, odom, odom_front, lane, stanley_flag):
        """
...
class SpeedDecision:
    ...
    def velocity_scale_path_dict(self, path_idx, add_speed, ref_velocity):
        velocity_scale_map = {0: (18.0 + add_speed) / ref_velocity, 1: (16.0 + add_speed) / ref_velocity, 2: (16.0 + add_speed) / ref_velocity, 3: (16.0 + add_speed) / ref_velocity}
            ...
            for i in [0, 1]:
                if i == path_path_index:
                    if gps_covariance < 0.01:
                        self.node.get_logger().info('GPS STATUS 0 GO ' + str(gps_covariance))
                        return True
                elif gps_covariance < 0.005:
                    self.node.get_logger().info('GPS STATUS 0 GO ' + str(gps_covariance))
                    return True
            self.node.get_logger().info('GPS STATUS 0 STOP ' + str(gps_covariance))
            return False
        return True
...
class SteeringDecision:
    ...
    def __init__(self, node: Node, numGlobalPathIn, numGloablPathOut, globalPathIn, globalPathOut):
        ...
        self.lfd_base_values = {0: (0.3841, 5.85), 1: (0.675, 2.875), 2: (0.65, 2.25), 3: (0.675, 2.875)}
    ...
    def stanley(self, velocity, heading, stanley_param_in, stanley_param_out, lane):
        try:
            if lane == 1:
                dx, dy, yaw = (stanley_param_in[0], stanley_param_in[1], stanley_param_in[2])
            else:
                dx, dy, yaw = (stanley_param_out[0], stanley_param_out[1], stanley_param_out[2])
            e = -(dx * cos(yaw + pi / 2) + dy * sin(yaw + pi / 2))
            theta_error = yaw - heading
            theta_error = atan2(sin(theta_error), cos(theta_error))
            velocity = max(6, velocity)
            theta_s = theta_error + atan2(0.15 * e, velocity / 3.6)
            return theta_s
        except Exception as e:
            self.node.get_logger().error('Error in Stanley method: ' % (e,))
            return 0
...
class LineChanger:
    ...
    def __init__(self, node: Node):
        ...
        self.stop_line_timer = self.node.get_clock().now().nanoseconds // 1000000000
    ...
    def line_change_check(self, path_index, distance, target_velocity, lidar_front, lidar_side_front, lidar_side_back):
        ...
        self.node.get_logger().info(target_velocity)
        if path_index == 0 or path_index == 2 or path_index == 3:
            if target_velocity < 13 and 25 > lidar_front > 5 and (lidar_side_front > lidar_front + 5) and (lidar_side_back <= -15):
                self.time_recorder()
                if self.line_change_flag and self.node.get_clock().now().nanoseconds // 1000000000 - self.stop_line_timer > 1.0:
