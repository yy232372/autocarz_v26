...
class GPS:
    ...
    def __init__(self, node: Node):
        ...
        self.t = self.node.get_clock().now()
        self.odom = Odometry()
    ...
    def getGPSBack(self, data):
        if self.last_gps_in - self.t >= Duration(seconds=5):
            self.gps_status = 0
        else:
        ...
        if self.checkGPS:
            if self.checkGPSFront and self.last_gps_in - self.last_gps_inFront <= Duration(seconds=0.11) and (self.gps_statusFront == 2 or self.gps_covarianceFront < 0.005):
