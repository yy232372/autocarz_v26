...
void readPath(std::string globalPath_file) {
    ...
    if (!fin.is_open()) {
        RCLCPP_ERROR(rclcpp::get_logger("roi_out"), "Could not open the path file: %s", globalPath_file.c_str());
        return;
    }
    ...
    if (globalNum == 0) {
        RCLCPP_ERROR(rclcpp::get_logger("roi_out"), "Path file contains no valid 4-column rows: %s", globalPath_file.c_str());
        return;
    }

    RCLCPP_INFO(rclcpp::get_logger("roi_out"), "[roi_out.cpp] loaded %d path points", globalNum);
    checkReadPath = true;
}

void pc_cb(const sensor_msgs::msg::PointCloud2::SharedPtr input_pcl_raw){

    // 기존 코드는 odom 없이도 기본값 htMat으로 ROI를 계산했다.
        ...
        for(unsigned int i=0; i<XYZ_array.size(); i++) {
            if ( XYZ_array.points[i].z <z_threshold ){
                if ( fabs(XYZ_array.points[i].y) >0.5 ||  fabs (XYZ_array.points[i].x) > 0.7 ){
                    ...
                    for (l = 0 ; l < MAX_BUFF && way_indx[l]!=-100 ; l++) {
                        w_indx=way_indx[l];
                        ...
                        if(bool_check>0) {
                            // XYZ_array.points[i].intensity=10;
    ...
    // ---------------------------------------------------------------------------------------------
        ...
        // 점 좌표 자체는 변환하지 않았으므로 원 센서 frame/stamp를 유지한다.
...
void odom_cb(const nav_msgs::msg::Odometry::SharedPtr odometry){
    // RCLCPP_INFO(rclcpp::get_logger("roi_out"), "odom_cb");
    // 고주기 콜백은 기본 INFO 터미널을 채우지 않도록 DEBUG로만 남긴다.
    RCLCPP_DEBUG(rclcpp::get_logger("roi_out"), "odom_cb");
    if(checkReadPath) {
...
void waypointInfo_cb(const autocarz::msg::WaypointInfo::SharedPtr info){
    ...
    lane = info->lane;
    if(checkReadPath) {
        int cur_way=info->indx_out;
        if(info->lane==2 || is_livox ) {   //forward 
...
public:
    RoiNode()
        : Node("roi_out")
    {
        std::string path_value = this->declare_parameter<std::string>("path_out", "");
        ...
        const auto roi_output_topic = this->declare_parameter<std::string>("topics.roi", "roi_out");
        const auto max_roi_topic = this->declare_parameter<std::string>("topics.max_roi", "max_roi_dis_out");
...
int main(int argc, char ** argv)
{
    ...
    auto node = std::make_shared<RoiNode>();
    rclcpp::spin(node);
