...
void readPath(std::string globalPath_file) {
    ...
    if (!fin.is_open()) {
        RCLCPP_ERROR(rclcpp::get_logger("roi_out_error"), "Could not open the path file: %s", globalPath_file.c_str());
        return;
    }
    ...
    if (globalNum == 0) {
        RCLCPP_ERROR(rclcpp::get_logger("roi_out_error"), "Path file contains no valid 4-column rows: %s", globalPath_file.c_str());
        return;
    }

    RCLCPP_INFO(rclcpp::get_logger("roi_out_error"), "[roi_out_error.cpp] loaded %d path points", globalNum);
    checkReadPath = true;
}
...
void odom_cb(const nav_msgs::msg::Odometry::SharedPtr odometry){
    RCLCPP_INFO(rclcpp::get_logger("roi_out_error"), "odom_cb");
    if(checkReadPath) {
...
class RoiOutErrorNode : public rclcpp::Node
{
public:
    RoiOutErrorNode()
        : Node("roi_out_error")
...
int main(int argc, char ** argv)
{
    ...
    rclcpp::spin(std::make_shared<RoiOutErrorNode>());
    rclcpp::shutdown();
    return 0;
}
