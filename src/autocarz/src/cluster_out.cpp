...
// ----------------- Clustering --------//
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("cluster_out"), "input points: %zu", num_points);
        ...
        duration_hz = duration_time > 0.0 ? 1.0 / duration_time : 0.0;
        RCLCPP_DEBUG(rclcpp::get_logger("cluster_out"),
            "cluster time: %.6f, hz: %.3f, max ID: %d",
            duration_time, duration_hz, Max_ID);
