...
class AutonomousVehicleController(Node):
    ...
    def __init__(self):
        ...
        self.lane = 1
        numGlobalPathIn = 1125
        numGlobalPathOut = 1123
    ...
    def run(self):
        ...
        while rclpy.ok():
            gps_status, gps_covariance, checkGPS, last_gps_in = self.gps_sensor.gps_data_back()
            ...
            checkERP = True
            if checkGPS and checkIMU and checkHeading and checkERP:
                ...
                gps_error_status = self.speed_decision.isGPSerror(gps_status, gps_covariance, last_gps_in, curr_path_idx)
                if gps_error_status == 'gps_error':
                    self.get_logger().warning('Both GPS Error. Stop.')
                    continue
                imu_error_status = self.speed_decision.isIMUerror(imu_covariance, last_imu_in, curr_path_idx)
