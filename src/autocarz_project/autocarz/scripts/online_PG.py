...
class SpeedDecision:
    ...
    def velocity_scale_path_dict(self, path_idx, add_speed, ref_velocity):
        velocity_scale_map = {0: (18.0 + add_speed) / ref_velocity, 1: (16.0 + add_speed) / ref_velocity, 2: (18.0 + add_speed) / ref_velocity, 3: (18.0 + add_speed) / ref_velocity}
...
class SteeringDecision:
    ...
    def calculate_lfd(self, velocity, path_idx, lane):
        path_idx = int(path_idx)
        K = self.K_in[path_idx] if lane == 1 else self.K_out[path_idx]
        if K < 2.5:
            K = 0
        lfd = 6 + velocity * 0.1 - sqrt(K) * 0.5
        return max(lfd, 3)
    ...
    def purePursuit(self, odom, velocity, heading, e, optimal_path, curr_path_idx):
        steering = 0
        curX = odom.pose.pose.position.x
        curY = odom.pose.pose.position.y
        max_lad_candidate = {0: 8.0, 1: 5.0, 2: 6.0, 3: 5.0, 4: 5.0}
        curr_max_lad = max_lad_candidate.get(curr_path_idx, 5.0)
        if optimal_path is None or len(optimal_path.poses) == 0:
            return 0
        print('E: ', e)
        weight = e / 3.4 * 2.0
        lfd = min(max(curr_max_lad - weight, 2.0), 10.0)
        print('LAD: ', lfd)
        cos_heading = cos(heading)
        sin_heading = sin(heading)
        found_valid_index = False
        for i in range(len(optimal_path.poses)):
            dx = optimal_path.poses[i].pose.position.x - curX
            dy = optimal_path.poses[i].pose.position.y - curY
            rx = cos_heading * dx + sin_heading * dy
            ry = sin_heading * dx - cos_heading * dy
            if rx > 0.0:
        ...
        if not found_valid_index:
            self.lfdPointidx = len(optimal_path.poses) - 1
        self.lfdPointidx = min(max(self.lfdPointidx, 0), len(optimal_path.poses) - 1)
        lfdPoint = Odometry()
        ...
        lfdPoint.pose.pose.position.x = optimal_path.poses[self.lfdPointidx].pose.position.x
        lfdPoint.pose.pose.position.y = optimal_path.poses[self.lfdPointidx].pose.position.y
...
class LineChanger:
    ...
    def line_change_check(self, path_index, distance, target_velocity, lidar_front, lidar_side_front, lidar_side_back):
        ...
        if path_index == 3:
            if 28 > lidar_front > 5 and lidar_side_front > lidar_front + 5 and (lidar_side_back <= -15):
                self.time_recorder()
                if self.line_change_flag and self.node.get_clock().now() - self.stop_line_timer > Duration(seconds=0.2):
                    self.node.get_logger().info('min_v line change start')
                    self.line_change(target_velocity)
                    is_change = True
            else:
                self.line_change_init()
