...
class WindowClass(QMainWindow, form_class) :
    def __init__(self, node: Node):
        ...
        # ──────────────── Existing state variables (GPS Hz 계산용) ──────
        ...
        # ──────────────── Existing subscribers (속도 + GPS) ─────────────
        self.sub_velocity = self.node.create_subscription(CtrlCmd, _topic(self.node, '/ctrlCmd'), self.velocity_status_text, CONTROL_QOS)
        self.sub_ublox1_status = self.node.create_subscription(NavSatFix, _topic(self.node, '/ublox1/fix'), self.for_reargps_status_text, qos_profile_sensor_data)
        self.sub_ublox2_status = self.node.create_subscription(NavSatFix, _topic(self.node, '/ublox2/fix'), self.for_frontgps_status_text, qos_profile_sensor_data)
    ...
    def for_reargps_status_text(self, data):
        ...
        if self.rear_current_time != 0 and self.rear_current_time != self.rear_previous_time:
            ...
            self.rear_gps_hz = "{}".format(int(hz))
            self.rear_msg_count = 0

        # if gps status =! 2 background become red
        if self.rear_gps_status != 2:
             self.rear_hz_color = QColor(255, 0, 0)
        else:
             self.rear_hz_color = QColor(0, 0, 0)
    ...
    def for_frontgps_status_text(self, data):
        ...
        if self.front_current_time != 0 and self.front_current_time != self.front_previous_time:
            ...
            # publish hz
            self.front_gps_hz= "{}".format(int(hz))

            self.front_msg_count = 0

        # if gps status =! 2 background become red
        if self.front_gps_status != 2:
             self.front_hz_color = QColor(255, 0, 0)
        else:
             self.front_hz_color = QColor(0, 0, 0)
    ...
    def _update_new_ui(self):
        # 1. main.py alive 판정 (waypointInfo 1초 이내 수신 = alive)
        is_main_alive = False
        if self.last_waypoint_msg_time is not None:
            dt = (self.node.get_clock().now() - self.last_waypoint_msg_time).nanoseconds * 1e-9
            is_main_alive = (dt < 1.0)
        ...
        # 3. main.py 꺼짐 → 차 이모지/waypoint 모두 비움
