...
class SpeedDecision:
    ...
    def __init__(self, node: Node):
        ...
        raw = _get_parameter(self.node, '/mode/velocity_scale_base')
        if isinstance(raw, str):
            try:
                raw = ast.literal_eval(raw)
            except (SyntaxError, ValueError) as exc:
                raise ValueError('mode.velocity_scale_base must be a dictionary-form string, for example: \'{"0": 20.0, "1": 18.0}\'') from exc
        if isinstance(raw, (list, tuple)):
            raw = {index: value for index, value in enumerate(raw)}
        if not isinstance(raw, dict):
            raise TypeError('mode.velocity_scale_base must resolve to dict, list, tuple, or dictionary-form string')
        self._velocity_scale_base = {int(k): float(v) for k, v in raw.items()}

    def velocity_scale_path_dict(self, path_idx, add_speed, ref_velocity):
        base = self._velocity_scale_base.get(int(path_idx))
        if base is None:
            return 'path_path_index_error'
        return (base + add_speed) / ref_velocity
    ...
    def control_speed(self, lidar_d_front, path_idx, velocity_scale_side):
        ...
        max_v = 20.0 * velocity_scale_side
        min_d = 8.0
        max_d = 23.0
    ...
    def control_speed_curve(self, lidar_d_front, path_idx, velocity_scale_side):
        ...
        velocity_scale_path = self.velocity_scale_path_dict(path_idx, add_speed, ref_velocity)
        min_v = 8.0 * velocity_scale_side
        max_v = 18.0 * velocity_scale_side
        min_d = 8.0
        max_d = 15.0
        criteria_d = (min_d + max_d) / 2.0
        criteria_v = (min_v + (max_v - min_v) * (criteria_d - min_d) / (max_d - min_d)) * velocity_scale_side * velocity_scale_path
...
class LineChanger:
    ...
    def line_change_check(self, path_index, distance, target_velocity, lidar_front, lidar_side_front, lidar_side_back, cur_lane):
        ...
        if path_index == 0:
            if cur_lane == 2:
                if lidar_front >= 15 and lidar_side_front >= 25 and (lidar_side_back <= -15):
                    self.time_recorder()
                    if self.line_change_flag and self.node.get_clock().now() - self.stop_line_timer > Duration(seconds=3.0):
                        self.line_change(target_velocity)
                        is_change = True
                elif target_velocity < 13 and 25 > lidar_front > 5 and (lidar_side_front > lidar_front + 5) and (lidar_side_back <= -15):
                    self.time_recorder()
    ...
    def lane_changing(self, target_velocity, distance):
        ...
        if target_velocity > self.temp_velocity:
            target_velocity = max(self.temp_velocity, 20)
        if distance < 0.3:
            done_flag = True
