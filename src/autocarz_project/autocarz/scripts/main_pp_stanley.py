...
from sensor_data_stanley_scenario import LidarSensor, GPS, VehicleState
...
class AutonomousVehicleController(Node):
    ...
    def __init__(self):
        ...
        self.lane = 1
        numGlobalPathIn = 1474
        numGlobalPathOut = 1535
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            if (checkGPS_front or checkGPS_back) and checkHeading and checkERP:
                ...
                curWaypointIndxIn, curWaypointIndxOut, distanceIn, distanceOut, stanley_param_in, stanley_param_out = self.path_planner.localization_pp_stanley(odom_back, odom_front, self.lane, stanley_flag)
                ...
                if change:
                    new_change_time = self.get_clock().now() + Duration(seconds=0.5)
                    self.lane = 1 if self.lane == 2 else 2
                if curr_path_idx == 1 or curr_path_idx == 2 or curr_path_idx == 3:
                    target_velocity, brake = self.speed_decision.control_speed_curve(lidar_front, curr_path_idx, 1.0)
                else:
                    target_velocity, brake = self.speed_decision.control_speed(lidar_front, curr_path_idx, 1.0)
                if is_line_changing:
                    target_velocity = min(target_velocity, 15)
                if stanley_flag:
                    steering_angle = self.steering_decision.stanley(velocity, heading, stanley_param_in, stanley_param_out, self.lane)
                else:
                    steering_angle = self.steering_decision.purePursuit(odom_back, self.lane, velocity, heading, curr_path_idx, curWaypointIndxIn, curWaypointIndxOut)
