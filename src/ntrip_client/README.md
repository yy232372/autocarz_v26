...
This repository version publishes `rtcm_msgs/msg/Message` and is connected to
the ROS 2 `ublox_gps` driver. Build it with `colcon build --symlink-install`.

Run both vehicle receivers and the NTRIP client without storing credentials:

```bash
ros2 launch ntrip_client autocarz_gnss.launch.py \
  username:="$NTRIP_USERNAME" password:="$NTRIP_PASSWORD"
```

The default topics are `/ublox1/fix`, `/ublox2/fix`, `/ublox1/nmea`, and
`/ntrip_client/rtcm`. Device paths live in `autocarz_gps1.yaml` and
`autocarz_gps2.yaml` under the `ublox_gps/config` directory.
