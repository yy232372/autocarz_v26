...
class ISRO:
    ...
    def getIMU(self, data):
        ...
        self.heading = atan2(2.0 * (self.orient_q.w * self.orient_q.z + self.orient_q.x * self.orient_q.y), 1.0 - 2.0 * (self.orient_q.y * self.orient_q.y + self.orient_q.z * self.orient_q.z))
        self.checkHeading = True
        self.last_imu_in = self.node.get_clock().now()
        self.imu_covariance = (0.0, 0.0, data.orientation_covariance[8])
        self.checkIMU = True
    ...
    def make_odom(self):
        ...
        if self.checkGPS and max(self.gps_covariance) < 0.5:
            self.odom.pose.pose.position.x = self.gpsX
            self.odom.pose.pose.position.y = self.gpsY
            self.odom.pose.pose.position.z = 0.0
        if self.checkIMU and max(self.imu_covariance) < 0.05:
            self.odom.pose.pose.orientation = self.orient_q
        if self.last_gps_in - self.last_imu_in <= Duration(seconds=0.11):
            self.odomPub.publish(self.odom)
